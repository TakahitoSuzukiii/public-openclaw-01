...
- `tasks(id, title, instruction, priority, status, result, created_at, updated_at, stage)` / `logs(id, task_id, ts, level, message)`。
...
- **カード**: 状態バッジ（実行待ちは専用表示）／#ID／**優先度インライン編集（数値＋保存）**／最新ログ／文脈依存ボタン（承認・実行完了・確認・完了・差し戻し・保留・再開・取消）。
