...
- ジムの記録アプリ（Hevy / Strong 等）のダッシュボード UX を参考に、KPI・推移・部位別・頻度・種目別トップを 1 画面で俯瞰できる構成にした。
...
### 2026-06-14: トップバー導線の見直し（追加修正・配線のみ）

- ホーム（`public/home.html`）・タスクボード（`public/index.html`）のトップバー「🏋 トレーニング」リンクの遷移先を、記録画面（`/training`）から **ダッシュボード（`/training/dashboard`）** に変更。「まずダッシュボードで俯瞰 → 必要なら記録へ」という導線に統一。
- ホーム画面の本文先頭に **「📊 ダッシュボード」「🏋 記録する」への導線パネル**を追加（既存スタイル `panel`/`cta`/`btn` を再利用、追加のみ）。
- ダッシュボード → 記録画面への導線は、ダッシュボード上部の既存リンク「🏋 記録する」（`/training`）を踏襲。
- HTML は毎リクエスト読み込みのため **再起動不要で反映**。本番 E2E（`/`・`/dashboard`・`/training/dashboard`・`/training`・`/api/training/stats`・`/info`・`/chat` すべて 200、既存デグレ無し）を確認。
- private バックアップ（`home.html`・`index.html`）も blob SHA 突合で **byte-exact 反映**済み。
